...
## 三、阶段4核心功能分组实施（Batch B-E）

### Batch B：触控模式 + 按键映射 + 平移缩放（优先级高，依赖少）

| Todo | 文件 | 复杂度 | 说明 |
|------|------|--------|------|
| TB-1 | `TouchModeSection` | 中 | 4个 Chip 横向排列（FilterChip），选中态联动 `engine.prefConfig`。经典模式关联"远程鼠标"按钮，触控板模式关联"双击按住"+"本地光标"开关。 |
| TB-2 | `KeyMappingSection` | 中 | Switch 连接 `engine.isCrownFeatureEnabled`。开启后 AnimatedVisibility 展开3个子入口：切换方案(`pageConfigController.open()`)、编辑方案(`elementController.changeMode(Edit)`)、其它设置(触控开关/灵敏度/滚轮/震动等)。 |
...
> **注意**：帧率切换需从 `PreferenceConfiguration` 读取当前值；显示器列表需通过 `conn.getDisplays()` 异步获取；码率滑块需复制原 `BitrateCardController` 的5段映射表。
...
| 修改 | `ui/SubPanelContainer.kt` | +`onOpenKeyboardShortcuts` 回调；实现 TB-1~TE-2 全部空壳 |
